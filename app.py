...
if uploaded_file:
    try:
        # Support Sheet Selection
        xl = pd.ExcelFile(uploaded_file)
        sheet_names = xl.sheet_names
        
        col_s1, col_s2 = st.columns(2)
        selected_sheet = col_s1.selectbox("Select Excel Sheet:", sheet_names, index=len(sheet_names)-1 if len(sheet_names) > 1 else 0)
        ...
        # Convert column to numeric, coercing text/headers to NaN and dropping them
        clean_series = pd.to_numeric(df_raw[selected_col], errors='coerce').dropna().reset_index(drop=True)
        
        if clean_series.empty:
            st.error("Selected column does not contain valid numeric data. Please choose another column or sheet.")
        else:
            data = clean_series
            df = pd.DataFrame({'Run Number': range(1, len(data) + 1), 'Value': data})
            
            st.write("**Processed Numeric Data Preview:**")
            ...
            with tab1:
                ...
                for decision, color in color_map.items():
                    ...
                    if not subset.empty:
                        fig1.add_trace(go.Scatter(
                            x=subset.index, y=subset['Value'], mode='markers',
                ...
                fig1.update_layout(xaxis_title="Run Number", yaxis_title="Value", hovermode="x unified")
